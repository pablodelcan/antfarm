...
function digCell(state, x, y) {
  ...
  if (state.grid[i] <= 1) { state.grid[i] = 0; state.totalDug++; state.terrainDirty = true; return 1; }
...
function aiAwarenessPass(state) {
  ...
  for (const ant of state.ants) {
    ...
    // Colony needs diggers
    else if (diggerCount < totalAnts * 0.4 && tunnelPct < 0.2 && ant.role === 'idle') {
      const assignment = tunnelPlanGetDigAssignment(tp, gx, gy, state.ants);
      if (assignment) {
        ant.digTargetX = assignment.gx;
        ant.digTargetY = assignment.gy;
...
function antEnter(state, ant, s, spd) {
  ...
  if (ant.targetX >= 0) {
    ...
    if (Math.abs(dx) > CELL * 3) {
      ...
      // FIX: Snap x to EXACT entrance position before descending
      // This prevents the V-shape caused by ants entering at slightly different x
      const entrance = tunnelPlanNearestEntrance(tp, ant._gx());
      if (entrance) {
...
function antDigTunnel(state, ant, s, spd) {
  ...
  // FIX: Shaft digging: force EXACTLY vertical
  if (isDrilling) {
    ant.digAngle = Math.PI * 0.5;
    // FIX: Also clamp X position to mainShaftX to prevent V-shape
    const nearShaft1 = Math.abs(ant._gx() - tp.mainShaftX) < 5;
    const nearShaft2 = tp.mainShaftX2 > 0 && Math.abs(ant._gx() - tp.mainShaftX2) < 5;
    if (nearShaft1) {
      // Gently pull ant to exact shaft center
      const targetX = tp.mainShaftX * CELL + CELL / 2;
      ant.vx += (targetX - ant.x) * 0.15;
    } else if (nearShaft2) {
      const targetX = tp.mainShaftX2 * CELL + CELL / 2;
      ant.vx += (targetX - ant.x) * 0.15;
    }
  }
  ...
  if (ant.digCD <= 0) {
    ...
    if (isSolid(state, fwdX, fwdY)) {
      ...
      if (hardness >= 5) {
        ...
        // FIX: Narrow dig radius near surface (first 10 rows) to prevent V-shape
        const depthBelowSurface = s.gy - SURFACE;
        let digRadius;
        if (isDrilling && depthBelowSurface < 10) {
          digRadius = 1; // narrow at surface
        } else if (isDrilling) {
          digRadius = 1.5; // wider deeper
        } else {
          digRadius = 1; // galleries stay narrow
        }
        let dugCount = 0;
...
function antGoUp(state, ant, s, spd) {
  ...
  for (let a = -Math.PI; a < Math.PI; a += 0.35) {
    ...
    if (!isSolid(state, cx, cy)) {
      const upBias = -Math.sin(a) * 3.0;
      const trailBonus = phGet(state, cx, cy, 'phTrail') * 1.0;
      const sc = upBias + trailBonus;
      if (sc > bestS) { bestS = sc; best = a; }
    }
  }
  ant.heading = ant.heading * 0.5 + best * 0.5;
  ant.vx += Math.cos(ant.heading) * spd * 0.35;
  ant.vy += Math.sin(ant.heading) * spd * 0.35;
  if (ant.stuck > 20 && ant.digCD <= 0) {
    digCell(state, s.gx, s.gy - 1); digCell(state, s.gx, s.gy - 2);
    ant.digCD = 8;
  }
  ant.vx *= 0.85; ant.vy *= 0.85;
}
...
function antAct(state, ant, s) {
  ...
  if (ant.stuck > 12) { antUnstick(state, ant, s.gx, s.gy); ant.stuck = 0; }
