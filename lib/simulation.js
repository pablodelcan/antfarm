...
function antGoUp(state, ant, s, spd) {
  ...
  ant.vy += Math.sin(ant.heading) * spd * 0.45;
  // Emergency dig upward much sooner (stuck > 6 instead of 20)
  if (ant.stuck > 6 && ant.digCD <= 0) {
    digCell(state, s.gx, s.gy - 1);
    digCell(state, s.gx, s.gy - 2);
    digCell(state, s.gx - 1, s.gy - 1);
    digCell(state, s.gx + 1, s.gy - 1);
    ant.digCD = 5;
  }
...
function antAct(state, ant, s) {
  ...
  // Ant-ant repulsion — prevent clustering
  for (const other of state.ants) {
    if (other === ant) continue;
    const dx = ant.x - other.x, dy = ant.y - other.y;
    const dist = Math.hypot(dx, dy);
    if (dist < CELL * 3 && dist > 0.1) {
      const force = (CELL * 3 - dist) * 0.03;
      ant.vx += (dx / dist) * force;
      ant.vy += (dy / dist) * force;
    }
  }
